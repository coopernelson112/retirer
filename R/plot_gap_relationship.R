#' Plot the cross-sectional relationship between two sex gaps
#'
#' Plot the directed sex gap in one measure against the directed sex gap in
#' another measure for a single year, either across OECD/EU membership groups or
#' across selected countries.
#'
#' @param x_measure Character scalar naming the sex-comparable measure to place
#'   on the x-axis.
#' @param y_measure Character scalar naming the sex-comparable measure to place
#'   on the y-axis.
#' @param year Optional numeric scalar giving the year to plot. If `NULL`, the
#'   latest year with overlapping non-missing gap data for both measures and the
...
#'
#' @export
plot_gap_relationship <- function(
  x_measure = "LE65",
  y_measure = "retirement_age",
  year = NULL,
  ...
  plot_df <- prep_gap_relationship_data(
    data = retirement,
    x_measure = x_measure,
    y_measure = y_measure,
    year = year,
  ...
  p <- ggplot2::ggplot(
    ...
    ggplot2::labs(
      x = measure_gap_label(x_measure, gap = gap),
      y = measure_gap_label(y_measure, gap = gap),
      colour = NULL
    ) +
    ggplot2::theme(
